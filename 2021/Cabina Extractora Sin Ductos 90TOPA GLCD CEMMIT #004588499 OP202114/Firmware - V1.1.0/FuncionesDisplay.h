...
void displayTiempo(int8 digito, int x, int y, int8 sombreado, int8 tamano)
{
   char voltage[9];
   sprintf(voltage, "%i", digito);
...
void displayTiempo2(int8 digito, int x, int y, int8 sombreado, int8 tamano)
{
   char voltage[9];
   sprintf(voltage, "%03u", digito);
   
   if(sombreado==1)
      {glcd_rect(x, y, x+(tamano*18), y+(tamano*8), YES, ON);glcd_text57(x+2, y+1, voltage, tamano, OFF);}
   if(sombreado==0)
      {glcd_rect(x, y, x+(tamano*18), y+(tamano*8), YES, OFF);glcd_text57(x+2, y+1, voltage, tamano, ON);}
...
void displayInt16(int16 digito, int x, int y, int sombreado, int tamano)
{
   char voltage[9];
   
   if(sombreado==2)
   {
      sprintf(voltage, "%05Lu", digito);
   ...
   else
   {
      ...
      if(sombreado==1)
         {glcd_rect(x, y, x+(tamano*30), y+(tamano*8), YES, ON);glcd_text57(x+1, y+1, voltage, tamano, OFF);}
      if(sombreado==0)
         {glcd_rect(x, y, x+(tamano*30), y+(tamano*8), YES, OFF);glcd_text57(x+1, y+1, voltage, tamano, ON);}
   }
   glcd_update();
}
