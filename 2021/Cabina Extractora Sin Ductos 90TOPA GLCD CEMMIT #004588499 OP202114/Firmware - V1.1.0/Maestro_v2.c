//Programa para Cabina Extractora de gases y Humos sin Ductos con variador y comunicacion MODBUS RTU-RS385
...
short Flanco=0,Flanco1=0,Flanco2=0,Flanco3=0,Flanco4=0,Flanco5=0,imprimir=0,flag_blower=0,flag_toma=0,flag_luz=0,w=0,Alarmas=OFF;
short tiempo_purga=0,tiempo_trabajo=0,flag_latencia=0,tiempo_postpurga=0,flagError=OFF;
short guardatrabajo=0,Lectura=0,n=0;
char  JP[] = "JP Inglobal", Cabina[] = "Cabina Extractora",Dos_puntos[]=":";
char  Clase[] = "Sin Ductos", Tiempo[] = "Tiempo de Purga";
char  Inflowt[] = "Vel:",Downflowt[] = "Temperatura", ZF[] = "Fabrica:208.0", ZA[] = "Actual:",AJ[]="Ajuste:";
char  Vida_Filtro[] = "T. Filtro:",ZFP[] = "Fabrica:235.0";
int8 Menu=200, unidad=1, Opcion=1,negativo=10,G_l=0,G_h=0,tiemporeset=0;
int8 l=0,h=0,Entero=0,Decimal1=0,Alarma=10,instruccion=0,address=0,address2=0,regl=0,regh=0,pwm1l=0,pwm1h=0,pwm2l=0,pwm2h=0,conteoError=0;

unsigned int16 Inflow_adc=0,Filtro_Downflow_adc=0,Temporal1=0;

int16 t_latencia=0,minutos_uv=0,minutos_trabajo=0,G16=0,tup=0,tdown=0,tright=0,tleft=0,reg=0,pwm1=0,pwm2=0;
...
float Inflow=0.0,Filtro_Downflow=0.0,Diferencia=0.0;
float Velocidad=0.0,x_uno=0.0,Ajuste1=0.0,Ajuste2=0.0;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      if(Menu==2){ //Menu General.
         ...
         if(VelMotor<32 && VelMotor>0){
         ...
         }else if(VelMotor<48 && VelMotor>=32){
            glcd_rect(0, 29, 13, 39, YES, ON);
            ...
            glcd_rect(60, 17, 73, 39, NO, ON);
            glcd_rect(75, 14, 88, 39, NO, ON);
         }else if(VelMotor<64 && VelMotor>=48){
            glcd_rect(0, 29, 13, 39, YES, ON);
            glcd_rect(15, 26, 28, 39, YES, ON);
            ...
            glcd_rect(60, 17, 73, 39, NO, ON);
            glcd_rect(75, 14, 88, 39, NO, ON);
         }else if(VelMotor<80 && VelMotor>=64){
            glcd_rect(0, 29, 13, 39, YES, ON);
            glcd_rect(15, 26, 28, 39, YES, ON);
            glcd_rect(30, 23, 43, 39, YES, ON);
            ...
            glcd_rect(60, 17, 73, 39, NO, ON);
            glcd_rect(75, 14, 88, 39, NO, ON);
         }else if(VelMotor<96 && VelMotor>=80){
            glcd_rect(0, 29, 13, 39, YES, ON);
            glcd_rect(15, 26, 28, 39, YES, ON);
            glcd_rect(30, 23, 43, 39, YES, ON);
            glcd_rect(45, 20, 58, 39, YES, ON);
         ...
         if(imprimir==1){
            ...
            displayTiempo2(VelMotor,25,11,0,1);  
            ...
            if(!Alarmas){
               strcpy(Menus.Letra,"JPCEGH90PPTOPA-EXPP");
               displayMenu(Menus.Letra,5,0,1,1);
            }else{
               strcpy(Menus.Letra,"!Verifique Variador!");
      ...
      if(tright>=30){
         if(Flanco4 == 0){
            Flanco4 = 1;flag_blower=!flag_blower;
         }
      }else{
         Flanco4 = 0;
      }
      ...
      if(tleft>=30){//Si oprime boton de Blower.
         if(Flanco3 == 0){
            Flanco3 = 1;Menu=33;glcd_fillScreen(OFF);      //Limpia la pantalla
         }
      }else{
         Flanco3 = 0;
      }
      ...
      if(tdown>=30){//Si oprime boton de Luz Blanca.
         if(Flanco5 == 0){
            Flanco5 = 1;flag_luz=!flag_luz;
         }
      }else{
         Flanco5 = 0;
      }
      
      if(tup>=400){//Si oprime boton de Toma.
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if(tleft>=30){//Si oprime boton de Toma.
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      if(Menu==31){ //Menu de Mantenimiento.
         ...
         LuzBlanca_on;
            ...
            if(tup>=30 && tright>=30) // Reinicia Tiempo de Luz UV
            {
                  delay_ms(2000);
                  write_eeprom(12,0);
                  delay_ms(20);
            ...
            if(tdown>=30 && tright>=30) // Reinicia Tiempo de Trabajo
            {
                  delay_ms(2000);
                  write_eeprom(16,0);
                  delay_ms(20);
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      temporales=((float)VelMotor/(1.6666666))*10.0;
      strcpy(Menus.Letra,"Seleccion Vel");
      displayMenu(Menus.Letra,10,0,1,1);
      sprintf(tempo, "Reg:%05Lu  R1:%05Lu",(int16)temporales,make16(txbuf[3],txbuf[4]));
      displayMenu(tempo,0,20,0,1);
      
      displayTiempo2(VelMotor,40,40,1,2);
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      if(Menu==44){ //Menu de Parametros Variador         
         ...
         sprintf(tempo, "R0:%05Lu  R1:%05Lu", make16(txbuf[1],txbuf[2]),make16(txbuf[3],txbuf[4]));
         displayMenu(tempo,0,9,0,1);
         sprintf(tempo, "R2:%05Lu  R3:%05Lu", make16(txbuf[5],txbuf[6]),make16(txbuf[7],txbuf[8]));
         displayMenu(tempo,0,18,0,1);
         sprintf(tempo, "R4:%05Lu  R5:%05Lu", make16(txbuf[9],txbuf[10]),make16(txbuf[11],txbuf[12]));
         displayMenu(tempo,0,27,0,1);
         sprintf(tempo, "R6:%05Lu  R7:%05Lu", make16(txbuf[13],txbuf[14]),make16(txbuf[15],txbuf[16]));
         displayMenu(tempo,0,36,0,1);
         sprintf(tempo, "PWM1:%05Lu  Add:%03u", make16(pwm1h,pwm1l),address2);
         displayMenu(tempo,0,45,0,1);
         sprintf(tempo, "tx[0]:%03u tx[19]:%03u", txbuf[0],txbuf[19]);
         displayMenu(tempo,0,54,0,1);
         sprintf(tempo, "Lectura MODBUS");
         displayMenu(tempo,0,0,1,1);
