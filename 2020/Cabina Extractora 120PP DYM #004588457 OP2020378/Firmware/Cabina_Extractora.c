...
#endif
*/
#fuses HS,NOWDT,NOPROTECT,NOLVP,NODEBUG,NOSTVREN,NOPUT,NOCPD,NOWRT,NOIESO,NOFCMEN,NOPBADEN,NOWRTC,NOWRTB,NOEBTR,NOEBTRB,NOCPB,NOXINST,NOLPT1OSC,MCLR,NOBROWNOUT
...
#define   Pantalla_on       output_bit(PIN_B3,1)
#define   Pantalla_off      output_bit(PIN_B3,0)
...
int8 Entero=0,Decimal1=0,Entero1=0,l=0,l2=0,h=0,h2=0,MediaMovil=0,negativo=10,Automa=10,Manual=10,Semi=10,ValorSemi=0,Opcion=1;
...
int16 tiempos=0,t_latencia=0,tiempos2=0,Dif16=0,tmp=10;
...
//-------------------Menu20----------------------------------------------------------------  
   if(Menu==20){
      ...
      if(Lectura2==1){
         ...
         //vel=((float)tmp-130.0)/0.74;//Ecuacion nueva, debe variar entre 13% y 20% para dar un voltaje de 0-120VAC para el motor
         vel=tmp;
         if(tmp<96){
            printf(lcd_putc,"Ext:%03i%%",(int8)vel);
         }else{
            printf(lcd_putc,"        100.0%%        ");
         }
         #endif
